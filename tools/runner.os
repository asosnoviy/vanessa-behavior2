...
Процедура Декомпилировать(Знач Путь, Знач КаталогВыгрузки, Знач КорневойПутьВыходной = "", Знач КорневойПутьВходной = "", Знач СтрокаПодключения = "", Знач Пользователь = "", Знач Пароль = "", Знач ВерсияПлатформы = "") Экспорт
	...
	НастройкиИзФайла = ПрочитатьНастройкиФайлJSON(КорневойПутьПроекта, "./build/cache.txt");
		...
		СледитьЗаИзменениямиФайловВРабочемКаталоге(ЗначенияПараметровСборки, НастройкиИзФайла);
...
Функция Компилировать(Знач Путь, Знач КаталогВыгрузки, Знач СтрокаПодключения = "", Знач Пользователь = "", Знач Пароль = "", Знач ВерсияПлатформы = "", ТолькоКопировать = Ложь) Экспорт
	...
	//Попытка
		...
		СледитьЗаИзменениямиФайловВРепозиторииИсходников(ЗначенияПараметровСборки, НастройкиИзФайла);
		Если ТолькоКопировать = Ложь Тогда
			ЗаписатьФайлJSON(ОбъединитьПути(КорневойПутьПроекта, "./build/cache.txt"), НастройкиИзФайла);
		КонецЕсли;
		Возврат Истина;
	//Исключение
...
Процедура СледитьЗаИзменениямиФайловВРабочемКаталоге(Значение, КэшПутей, Фильтр = "")
	...
	Для каждого Файл из СписокФайлов Цикл
		...
		Если СписокФайлов.Количество() > 20 И СписокОбработанных.Количество() + 1 % Цел((СписокФайлов.Количество()*30/100)) = 0 Тогда 
			Сообщить("Обработано " + СписокОбработанных.Количество());
		КонецЕсли;
		...
		Если Время = Неопределено ИЛИ Время <> ВремяТекущее Тогда 
			Изменен = Истина;
			Лог.Информация("Изменен:"+Файл.ПолноеИмя + " время старое:"+Строка(Время) + " новое:"+ВремяТекущее);
		КонецЕсли;
