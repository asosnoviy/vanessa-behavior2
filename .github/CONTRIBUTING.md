...
* склонируйте репозитарий себе на машину ```git clone https://github.com/*ТУТИМЯВАШЕГОПОЛЬЗОВАТЕЛЯ*/vanessa-behavior-new.git ```
...
```
cd vanessa-behavior-new
opm run init
```
* в случаи, если база уже была проинициализированна, тогда можно просто запустить сборку обработок из новых данных:
```opm run cepf```
* в каталоге `vanessa-behavior-new\build` добавьте новый `feature-файл`
* отредактируйте при необходимости `vanessa-behavior-new\build\vanessa-behavior.epf` 
* разработайте step проверки в `vanessa-behavior-new\build\features\*`
...
* В гите проверить необходимые изменения и зафиксировать только их. Если меняли управляемую форму, не обязательно добавлять в фиксацию и файлы толстых форм, т.к. они при каждом сохранении могут поменятся. 
* Отправьте изменения на github ```git push origin feature/issue-9999```
* Сформируйте `pull-request`. 
