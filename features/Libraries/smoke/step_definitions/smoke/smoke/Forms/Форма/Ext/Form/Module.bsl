...
Функция ПолучитьСписокТестов(КонтекстФреймворкаBDD) Экспорт
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты, "ЯОткрываюФормуСправочникаДляНовогоЭлементаСИсключением(Парам01, Парам02)", "ЯОткрываюФормуСправочникаДляНовогоЭлементаСИсключением", "");
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты, "ЯОткрываюФормуСправочникаДляСуществующегоЭлементаСИсключением(Парам01, Парам02)", "ЯОткрываюФормуСправочникаДляСуществующегоЭлементаСИсключением", "И	Я открываю форму ""Catalogs.Companies.Form.ItemForm"" для существующего элемента");
...
Функция СоздатьЭлементИВернутьСсылку(ИД, Наименование, ПроверяемоеПравоДоступа = Неопределено)
...
Функция ТестДолжен_ОткрытьФормуСуществующегоЭлементаСправочникаСервер(Мета_ПолноеИмя)
	...
	СоздаваемыйЭлемент = СоздатьЭлементИВернутьСсылку(ИД, "существующий");
	Если СоздаваемыйЭлемент <> Неопределено Тогда
		СуществующийЭлемент = СоздаваемыйЭлемент;
	КонецЕсли; 
	Возврат СоздаваемыйЭлемент;
КонецФункции
...
&НаСервере
Процедура УдалитьСозданныйОбъект(CоздаваемыйЭлемент)
...
Процедура ЯОткрываюФормуСправочникаДляСуществующегоЭлементаСоСвойствомИЗначением(Мета_ПолноеИмя, ИмяСвойства="", Значение="") Экспорт
	...
	Если Не ЗначениеЗаполнено(СуществующийЭлемент) Тогда
		УдалитьСозданныйОбъект(СуществующийЭлемент);
...
&НаКлиенте
Процедура ЯОткрываюФормуСправочникаДляСуществующегоЭлементаСИсключением(Мета_ПолноеИмя, СтрокаИсключения) Экспорт 
...
&НаКлиенте
Процедура ЯОткрываюФормуСправочникаДляНовогоЭлементаСИсключением(Мета_ПолноеИмя, СтрокаИсключения) Экспорт 
	
	Попытка
		ЯОткрываюФормуДокументаДляНовогоЭлементаСоСвойствомИЗначением(Мета_ПолноеИмя);
		ВызватьИсключение "Исключение при открытии нет " + СтрокаИсключения;
	Исключение
		Ошибка = ИнформацияОбОшибке();
		ПодробноеОписание = ПодробноеПредставлениеОшибки(Ошибка);
...
&НаКлиенте
Процедура ГенерацияПоПодстемам(Команда)
	МассивТекстов=Новый Массив;
	ГенерацияПоПодстемамНаСервер(МассивТекстов);
...
Процедура ДобавитьТекстДляКаждойПроизвольнойФормы(ТекстовыйДокумент, Формы, МассивФорм)
	
	Для каждого Форма Из Формы Цикл
		Если МассивФорм.Найти(Форма) = Неопределено Тогда
			ТекстовыйДокумент.ДобавитьСтроку("		И	Я открываю произвольную форму '" + Форма.ПолноеИмя() + "'");						
		КонецЕсли; 
	КонецЦикла;

&НаСервере
Процедура ГенерацияПоПодстемамНаСервер(МассивТекстов)
...
|");
		...
		Для каждого Элемент Из Состав Цикл
			...
			Если ЭтоСправочник Тогда
				...
				ДобавитьТекстДляКаждойПроизвольнойФормы(Текст, Элемент.Формы, МассивФорм)
