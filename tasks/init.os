...
Процедура ИнициализацияОкружения()
	...
	Лог.Информация("Создание сервисной базы init.os init-dev --dev");
	
	Команда = Новый Команда;
	Команда.УстановитьПравильныйКодВозврата(0);
	
	СтрокаВыполнения = "oscript ./tools/init.os init-dev --src ./lib/CF/83NoSync310";
	Лог.Информация(СтрокаВыполнения);
	Команда.УстановитьСтрокуЗапуска(СтрокаВыполнения);
	Команда.Исполнить();

	СтрокаВыполнения = "oscript ./tools/init.os init-dev --dev --src ./lib/CF/83NoSync310";
	Лог.Информация(СтрокаВыполнения);
	Команда.УстановитьСтрокуЗапуска(СтрокаВыполнения);
	Команда.Исполнить();
	...
	Если Аргументы.Команда = ВозможныеКоманды.file Тогда
		...
		Команда.УстановитьСтрокуЗапуска("oscript ./tools/init.os init-dev");
		Команда.Исполнить();
	...
	ИначеЕсли Аргументы.Команда = ВозможныеКоманды.server Тогда
		...
		Если Результат.Получить("default") <> Неопределено Тогда
			//Результат["default"]
			Результат["default"].Вставить("--ibname", СтрокаПодключения );
	...
	Если НЕ Новый Файл("./build/1Cv8.cf").Существует() Тогда
		...
		Команда.УстановитьСтрокуЗапуска("oscript ./tools/runner.os compile");
		Команда.Исполнить();
	Иначе
	...
	Команда.УстановитьСтрокуЗапуска("deployka loadcfg " + СтрокаПодключения + " ./build/ /mode -load");
	Команда.Исполнить();

	Команда.УстановитьСтрокуЗапуска("deployka loadcfg /F./build/ibservice ./build/ /mode -load");
	Команда.Исполнить();

	СистемнаяИнформация.УстановитьПеременнуюСреды("RUNNER_DBUSER", " ");
	СистемнаяИнформация.УстановитьПеременнуюСреды("RUNNER_DBPWD", " ");

	Лог.Информация("oscript runner.os updatedb");
	Результат = ЗапуститьИПодождать("oscript ./tools/runner.os updatedb --uccode test");
	...
	СтрокаЗапуска = "oscript ./tools/runner.os run --command VBParams=./tools/epf/init.json --execute ./build/tools/epf/init.epf";
	Лог.Информация(СтрокаЗапуска);
	ЗапуститьИПодождать(СтрокаЗапуска);
