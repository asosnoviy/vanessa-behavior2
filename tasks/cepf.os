...
Процедура ИнициализацияОкружения()
	...
	Если Не ПустаяСтрока(ОдинКаталог) Тогда
		...
		ШаблонЗапуска = СтрШаблон("oscript ./tools/runner.os compileepf %1 ./build/%2", ОдинКаталог, ВторойКаталог);
...
Процедура ЗапуститьОбработка(Знач Путь)
	...
	Сообщить(Файл.ИмяБезРасширения + ":"+Строка(Файл.ЭтоКаталог()));
	Если Файл.ИмяБезРасширения = "epf" И Файл.ЭтоКаталог() Тогда 
		ШаблонЗапуска = СтрШаблон("oscript ./tools/runner.os compileepf ./epf ./build/");
	Иначе
		ШаблонЗапуска = СтрШаблон("oscript ./tools/runner.os compileepf %1 ./build/%1", Путь);
	КонецЕсли;
	ШаблонЗапуска = ШаблонЗапуска + " --ibname /F./build/ibservice";
	Лог.Информация(ШаблонЗапуска);
	ЗапуститьИПодождать(ШаблонЗапуска);
