...
Процедура ИнициализацияОкружения()
	...
	текстЛога = "";
	ПолучитьТекстФайла(ИмяФайлаЛога, текстЛога);
...
Функция РаспарситьИнформациюОбКоммите(текстФайла)
	...
	Сообщение = "";
	ВерсияКоммита = "";
	МассивСтрок = СтроковыеФункции.РазложитьСтрокуВМассивПодстрок(текстФайла, Символы.LF);
	...
	Для Сч = 1 По МассивСтрок.ВГраница() Цикл
		...
		КонецЕсли;
		Строка = МассивСтрок.Получить(Сч);
		Разделитель = ?(ЗначениеЗаполнено(Сообщение), Символы.LF, "");
		Сообщение = Сообщение + Разделитель + СокрЛП(Строка);
	КонецЦикла;

	Сообщение = Сообщение + Символы.LF + МассивСтрок.Получить(0);
